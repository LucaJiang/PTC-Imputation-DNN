...
class GradCam:
    ...
    def __call__(self, input_img, target_category=None):
        ...
        my_grad_zero(self.feature_module)
        my_grad_zero(self.model)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(
        'visual.pt', map_location=torch.device('cpu')))

    imgs = []
    imgnames = []
    for imagename in os.listdir(args.image_path):
        img = cv2.imread(os.path.join(args.image_path, imagename))
        if img is not None:
            imgs.append(img)
    ...
    for i in range(len(imgs)):
        ...
        cv2.imwrite("cam\\"+imgname, cam)
        cv2.imwrite("gb\\"+imgname, gb)
        cv2.imwrite("cam_gb\\"+imgname, cam_gb)
